...
@pytest.fixture
def temp_env():
    """Create a temporary directory for input, output, and SQLite DB files."""
...
def test_full_pipeline(temp_env, capsys, caplog):
    ...
    # 2. Patch SQLITE_PATH so tests use an isolated temp DB (never touching production data)
    with patch('database.db.SQLITE_PATH', db_path):
